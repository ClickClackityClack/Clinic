...
router.post('/', function (req, res, next) {
    mongoClient.connect(url, {
        ...
        dbObj.collection('Patients').insertOne(newPatient, (err, res) => {
            if (err) throw err;
...
router.delete('/:id', function (req, res) {
    mongoClient.connect(url, {
        ...
        dbo.collection('Patients').deleteOne(param, (err, result) => {
            if (err) throw err;
            console.log(result);
            db.close();
        });
...
router.put('/:id', (req, res) => {
    mongoClient.connect(url, {
        ...
        dbo.collection('Patients').updateOne(query, newValues, (err, res) => {
            if (err) throw err;
    ...
    mongoClient.connect(url, {
        ...
        let query = {
            $or: [{
                ...
                {
                    zip: new RegExp(param, 'i')
                },
                {
                    phone: new RegExp(param, 'i')
                }, 
                {
                    DOB: new RegExp(param, 'i')
                }
            ]
        }
